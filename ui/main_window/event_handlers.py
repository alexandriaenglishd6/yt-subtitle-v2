...
class EventHandlersMixin:
    ...
    def _update_cookie_status(
        ...
        if cookie:
            if test_result == "failed":
                ...
                if region:
                    cookie_status = t(
                        "cookie_status_test_success_with_region", region=region
                    )
                else:
                ...
                if region:
                    cookie_status = t(
                        "cookie_status_configured_with_region", region=region
                    )
                else:
        ...
        if hasattr(self, "log_panel") and hasattr(
            self.log_panel, "update_cookie_status"
        ):
            self.log_panel.update_cookie_status(
                cookie_status, cookie=cookie, region=region, test_result=test_result
            )
    ...
    def _on_save_run_params(
        self, concurrency: int, ai_concurrency: int, retry_count: int, output_dir: str
    ):
        """保存运行参数

        Args:
            concurrency: 并发数
            ai_concurrency: AI 并发数
            retry_count: 重试次数
            output_dir: 输出目录
        """
    ...
    def _on_language_changed(self, value: str):
        ...
        if value == "English":
            ...
            logger.warning_i18n("log.language_unknown", value=value)
            return

        current_lang = get_language()

        if current_lang == new_lang:
            logger.info_i18n("log.language_no_change")
            return
        ...
        # 现在使用新语言记录日志
        logger.info_i18n("log.language_change_callback", value=value)
        logger.info_i18n("log.language_current_new", current=current_lang, new=new_lang)
        logger.info_i18n("log.language_set", lang=new_lang, test=t("app_name"))
        ...
        try:
            if hasattr(self.app_config, "ui_language"):
                ...
                self.config_manager.save(self.app_config)
                logger.info_i18n("log.language_saved")
        except Exception as e:
            logger.error_i18n("log.language_save_failed", error=str(e))
        ...
        if theme_name != self.current_theme:
            ...
            try:
                if hasattr(self.app_config, "theme"):
                    ...
                    self.config_manager.save(self.app_config)
                    logger.info_i18n("log.theme_saved", theme_name=theme_name)
            except Exception as e:
                logger.error_i18n("log.theme_save_failed", error=str(e))
    ...
    def _refresh_ui_texts(self):
        ...
        logger.info_i18n("log.ui_text_refreshed")
    ...
    def _on_open_output_folder(self):
        ...
        self.log_panel.append_log(
            "INFO", t("output_folder_opened", path=str(output_dir.absolute()))
        )
    ...
    def _on_open_failed_links(self):
        ...
        if not failed_urls_file.exists():
            ...
            self.log_panel.append_log(
                "INFO", t("failed_links_file_created", path=str(failed_urls_file.absolute()))
            )
        ...
        try:
            ...
            self.log_panel.append_log(
                "INFO", t("failed_links_file_opened", path=str(failed_urls_file.absolute()))
            )
        except Exception as e:
            self.log_panel.append_log(
                "ERROR", t("failed_links_file_open_failed", error=str(e))
            )
    ...
    def _on_cookie_status_changed(self, test_result: str):
        """Cookie 状态变化事件处理"""
        self.after(0, lambda: self._update_cookie_status(test_result=test_result))
