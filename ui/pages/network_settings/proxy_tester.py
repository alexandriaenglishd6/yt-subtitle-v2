        ...
        for i, proxy in enumerate(proxies, 1):
            ...
            try:
                ...
                if not yt_dlp_path:
                    ...
                    test_success, last_error, last_exception = self._test_proxy_with_requests(
                        proxy, test_proxy_url, test_urls, timeout, parsed, i, log_info, log_debug
                    )
                else:
                    # 使用 yt-dlp 测试代理
                    test_success, last_error, last_exception = self._test_proxy_with_ytdlp(
                        proxy, test_proxy_url, yt_dlp_path, timeout, parsed, i, log_info, log_debug
                    )
        ...
        for url_idx, test_url in enumerate(test_urls, 1):
            try:
                ...
                if response.status_code in [200, 301, 302, 303, 307, 308]:
                    ...
                    log_info(t("log.proxy_test_success_http", index=index, proxy=safe_proxy, test_url=test_url, status_code=response.status_code))
                    test_success = True
                    last_error = None
                    break
                else:
    ...
    def _test_proxy_with_ytdlp(
        ...
        try:
            ...
            if result.returncode == 0:
                safe_proxy = f"{parsed.scheme}://{parsed.username or ''}:***@{parsed.hostname}:{parsed.port or ''}" if (parsed.username or parsed.password) else proxy
                log_info(t("log.proxy_test_success_ytdlp", index=index, proxy=safe_proxy))
                ...
                safe_proxy = f"{parsed.scheme}://{parsed.username or ''}:***@{parsed.hostname}:{parsed.port or ''}" if (parsed.username or parsed.password) else proxy
                log_info(t("log.proxy_test_success_cookie_required", index=index, proxy=safe_proxy))
                ...
                if "ERROR:" in error_output and "youtube" in error_lower:
                    safe_proxy = f"{parsed.scheme}://{parsed.username or ''}:***@{parsed.hostname}:{parsed.port or ''}" if (parsed.username or parsed.password) else proxy
                    log_info(t("log.proxy_test_success_youtube", index=index, proxy=safe_proxy))
