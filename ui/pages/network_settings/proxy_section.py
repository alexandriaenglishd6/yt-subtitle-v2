...
class ProxySectionMixin(ProxyTesterMixin):
    ...
    def _build_proxy_section(self, parent_frame):
        ...
        proxy_label = ctk.CTkLabel(
            proxy_frame, text=t("proxy_label"), font=heading_font(weight="bold")
        )
        proxy_label.grid(
            ...
            try:
                ...
                def update_ui():
                    self.proxy_test_btn.configure(
                        state="normal", text=t("proxy_test_connection")
                    )
                    ...
                    if self.on_log_message:
                        ...
                        if success_count > 0:
                            self.on_log_message(
                                "INFO",
                                t(
                                    "log.proxy_test_success_summary",
                ...
                def on_error():
                    ...
                    if self.on_log_message:
                        self.on_log_message(
                            "ERROR", t("proxy_test_failed", error=error_str)
                        )
