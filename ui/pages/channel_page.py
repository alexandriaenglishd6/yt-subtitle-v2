...
from core.utils.url_validation import validate_youtube_url
...
class ChannelPage(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # URL 校验相关变量
        self.url_var = ctk.StringVar(value=self.initial_channel_url)
    ...
    def _build_ui(self):
        ...
        self._title_label = ctk.CTkLabel(
            self.scrollable_frame, text=t("channel_mode"), font=title_font(weight="bold")
        )
        ...
        self.url_error_label.pack(anchor="w", padx=160, pady=(0, 4))
        ...
        # 目标语言（输入框）- 放在下方
        subtitle_target_label = ctk.CTkLabel(
        ...
        # 摘要语言（带启用勾选框）
        summary_lang_label = ctk.CTkLabel(
        ...
        # 双语模式
        bilingual_mode_label = ctk.CTkLabel(
        ...
        # 翻译策略（带启用勾选框）
        translation_strategy_label = ctk.CTkLabel(
    ...
    def _on_url_changed(self, *args):
        ...
        if not url:
            self.channel_url_entry.configure(border_color=["#979DA2", "#565B5E"]) # 恢复默认
            self.url_error_label.configure(text="")
            return

        is_valid, error_key = validate_youtube_url(url)
        if not is_valid:
            self.channel_url_entry.configure(border_color="red")
            error_msg = t(f"log.{error_key}") if error_key else "Invalid URL"
        ...
        if hasattr(self, "stats_text"):
            ...
            if eta_seconds is not None and eta_seconds > 0:
                ...
                if eta_minutes > 0:
                    ...
                    stats_lines.append(
                        t("eta_remaining_seconds", seconds=int(eta_seconds))
                    )
            ...
            if error_counts:
                ...
                if error_summary:
                    stats_lines.append(
                        t("error_classification") + ": " + ", ".join(error_summary)
                    )

            self.stats_text.configure(text="\n".join(stats_lines), justify="left")
    ...
    def refresh_language(self):
        ...
        if hasattr(self, "channel_url_entry"):
            self.channel_url_entry.configure(
                placeholder_text=t("channel_url_placeholder")
            )
