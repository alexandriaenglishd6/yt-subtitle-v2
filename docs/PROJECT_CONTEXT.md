...
1. **导入项目**：将项目文件夹导入 IDE
2. **阅读文档**：
   - `docs/dev_log.md` - 开发历史（重点看 2026-01 部分）
...
**项目信息**：
- YouTube 字幕工具 v1.1.1，已发布
- 刚刚完成了 P0+P1+P2 重构和代理优化
- 下一步是 v2.0 高并发优化
- 技术栈：Python + CustomTkinter

**请先阅读以下文档**：
1. `docs/dev_log.md` - 开发历史（重点看 2026-01-20 日志）
...
**最近更新 (v1.1.1)**：
- P0：`single_video.py` 拆分为多阶段函数
- P1：提取 `subtitle_format` 和 `proxy_retry` 独立模块
- P2：单元测试覆盖率提升
- 修复：代理重试排除逻辑 (Exclude Tried Proxies)
```
...
| 决策 | 说明 |
|------|------|
| v1.1.1 重构 | 拆分大函数，提取独立模块，提升代码可维护性 |
| 代理重试策略 | 强制排除已尝试代理，增加重试间隔到 5-10s |
| 高并发优先 | v2.0 重点是性能优化 |
| 国际化规范 | 所有文本和日志都要国际化 |
