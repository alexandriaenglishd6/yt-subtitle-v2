...
| **2025-12-26** | **v1.0.1 发布** | **Bug 修复版本：统计显示、Cookie、AI 测试问题修复** | `README.md`, `core/version_checker.py` |
...
- **阶段 / 里程碑**：🎉 v1.1.1 发布 - 代码质量重构 + 代理重试优化
- **参与 AI / 工具**：Gemini Antigravity
- **完成的任务**：
  1. **P0 重构 - `single_video.py` 超大函数拆分**：
     - 将 377 行的 `process_single_video()` 拆分为 10 个独立函数
     - 引入 `StageContext` 数据类封装公共参数
     - 创建阶段函数：`_stage_detect()`、`_stage_download()`、`_stage_translate()`、`_stage_summarize()`、`_stage_output()`
     - 创建辅助函数：`_check_cancelled()`、`_check_incremental()`、`_handle_stage_error()`
  2. **P1a 重构 - 字幕格式转换模块提取**：
     - 新建 `core/subtitle_format.py`（约 200 行）
     - 包含：`convert_vtt_to_srt()`、`convert_json3_to_srt()`、`convert_srv3_to_srt()`、`detect_format()`、`convert_to_srt()`
     - 从 `downloader.py` 删除约 110 行重复代码
  3. **P1b 重构 - 代理重试工具模块创建**：
     - 新建 `core/proxy_retry.py`（约 200 行）
     - 包含：`ProxyRetryContext` 类、`@with_proxy_retry` 装饰器、`retry_with_proxy_rotation()` 函数
  4. **P2 单元测试补充**：
     - 新建 `tests/test_subtitle_format.py`（25 个测试用例）
     - 新建 `tests/test_proxy_retry.py`（14 个测试用例）
     - 39 个新测试全部通过
  5. **代理重试优化（方案 B）**：
     - `proxy_manager.py`：`get_next_proxy()` 添加 `exclude` 参数，支持排除已尝试的代理
     - `downloader.py`：重试时传入 `tried_proxies` 集合，确保每次重试使用不同代理
     - 重试延迟从 2-5 秒增加到 5-10 秒
  6. **版本发布 v1.1.1**：
     - 更新 `core/__init__.py`、`zh_CN.json`、`en_US.json` 版本号
     - 更新 `build.spec` 输出目录名
     - PyInstaller 打包成功，包含 `yt-dlp.exe`
- **主要修改点总结**：
  - **新增模块**：`core/subtitle_format.py`、`core/proxy_retry.py`
  - **重构模块**：`core/pipeline/single_video.py`（377 行 → 多函数）
  - **优化模块**：`core/proxy_manager.py`、`core/downloader.py`
  - **新增测试**：`tests/test_subtitle_format.py`、`tests/test_proxy_retry.py`
  - **版本更新**：4 个文件版本号更新到 v1.1.1
- **代码质量提升**：
  | 指标 | 重构前 | 重构后 |
  |------|--------|--------|
  | 最大函数行数 | 377 | ~100 |
...
- **测试结果**：
  - ✅ 39 个新增测试全部通过
  - ✅ 真实任务测试：2 个视频处理成功（代理重试换代理逻辑验证正确）
  - ✅ 打包后 `yt-dlp.exe` 已包含
- **Git 提交**：
  - `v1.1.1: P0+P1+P2重构 + 代理重试优化 + 版本更新`（18 files, +1995/-533）
- **下一步计划**：
  - 便携版分发测试
  - 后续功能开发
...
- **阶段 / 里程碑**：v1.0.1 Bug 修复版本发布
- **参与 AI / 工具**：Gemini Antigravity
- **完成的任务**：
  1. **统计显示修复**：
     - 修复 "计划" 数量在视频检测完成后不更新的问题（`batch.py` 中 `initial_url_count` 逻辑错误）
     - 修复 "失败" 数量始终显示为 0 的问题（`_detect_subtitles` 未传入 `on_stats` 回调）
     - 无字幕的视频现在正确计入失败数
  2. **Cookie 测试修复**：
     - 修复 Cookie 保存和检测失败的问题
  3. **AI 测试修复**：
     - 修复免费谷歌翻译测试问题
     - 修复 Kimi API 测试卡住的问题
  4. **URL 输入框优化**：
     - 实现真正的 placeholder 效果（灰色显示，点击自动消失）
     - 添加 `_show_placeholder`、`_hide_placeholder`、`_on_focus_in`、`_on_focus_out` 方法
  5. **版本发布**：
     - 更新版本号到 v1.0.1
     - 更新 README.md 更新日志
     - 打包发布到 GitHub
- **主要修改点总结**：
  - **核心修复**：`core/pipeline/batch.py`、`ui/business_logic/task_runner.py`、`ui/business_logic/processing_pipeline.py`
  - **UI 优化**：`ui/pages/url_list_page.py`（placeholder 效果）
  - **版本更新**：`core/version_checker.py`、`README.md`
- **下一步计划**：
  - 继续 v2.0 高并发优化阶段
