...
class LanguageConfig:
    ...
    subtitle_target_languages: list[str] = field(
    ...
    subtitle_format: Literal["srt", "txt", "both"] = (
        "srt"  # 字幕输出格式：srt（带时间轴）、txt（纯文本）、both（两种都输出）
    )
    ...
    def from_dict(cls, data: dict) -> "LanguageConfig":
        ...
        if "target_languages" in data and "subtitle_target_languages" not in data:
            ...
            logger.debug(
                "检测到旧字段名 'target_languages'，已转换为 'subtitle_target_languages'"
            )

        # 步骤 2：标准化语言代码
        target_langs = data.get("subtitle_target_languages", ["zh-CN"])
        normalized_target_langs = [
            normalize_language_code(lang) for lang in target_langs
        ]
