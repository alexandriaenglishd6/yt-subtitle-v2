...
def process_single_video(
    ...
    try:
        ...
        if not force:
            ...
            if status == "success":
                msg = logger.info_i18n(
                    "video_already_processed", video_id=video_info.video_id
                )
        ...
        if cancel_token and cancel_token.is_cancelled():
            ...
            msg = logger.info_i18n(
                "task_cancelled", reason=reason, video_id=video_info.video_id
            )
            safe_log(on_log, "INFO", msg, video_info.video_id)
        ...
        if not detection_result.has_subtitles:
            ...
            if translation_llm_init_error_type:
                error_msg = (
                    f"视频无字幕且翻译 LLM 初始化失败: {translation_llm_init_error}"
                )
        ...
        if (
            language_config.summary_enabled
            and summary_llm
            and download_result.get("original")
        ):
            if cancel_token and cancel_token.is_cancelled():
                raise TaskCancelledError()
