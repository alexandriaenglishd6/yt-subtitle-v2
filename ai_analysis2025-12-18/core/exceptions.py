...
class ErrorType(str, Enum):
    ...
    COOKIE_EXPIRED = "cookie_expired"  # Cookie 已失效或过期（需要用户更新）
...
class AppException(Exception):
    ...
    def __init__(
        self, message: str, *, error_type: ErrorType, cause: Optional[Exception] = None
    ):
    ...
    def __init__(self, reason: Optional[str] = None):
        ...
        if reason:
            message = translate_exception(
                "exception.task_cancelled_with_reason", reason=reason
            )
        else:
            message = translate_exception("exception.task_cancelled", reason="")
        super().__init__(message=message, error_type=ErrorType.CANCELLED)
        self.reason = reason
...
class LocalModelError(AppException):
    ...
    def __init__(self):
        ...
        super().__init__(message=message, error_type=ErrorType.EXTERNAL_SERVICE)
