...
class CookieManager:
        ...
        try:
            ...
            for key, value in cookies.items():
                ...
                if key_upper in ["GL", "LOCALE"]:
                    region = value.upper()
                    logger.info_i18n(
                        "cookie_region_extracted_field", key=key, region=region
                    )
                    return region
    ...
    def get_cookie_file_path(self) -> Optional[str]:
        ...
        if self._temp_cookie_file and self._temp_cookie_file.exists():
            logger.debug_i18n(
                "cookie_file_exists", cookie_file=str(self._temp_cookie_file)
            )
            return str(self._temp_cookie_file)

        # 创建新的临时文件
        self._temp_cookie_file = self._cookie_string_to_netscape_file(
            self.cookie_string
        )
        if self._temp_cookie_file:
            logger.debug_i18n(
                "cookie_file_created", cookie_file=str(self._temp_cookie_file)
            )
            return str(self._temp_cookie_file)
        else:
    ...
    def test_cookie(self, test_url: str = "https://www.youtube.com") -> Dict[str, any]:
        ...
        if not self.cookie_string:
            return {
                "available": False,
                "region": None,
                "error": translate_log("cookie_not_configured"),
                "details": {},
            }
        ...
        try:
            ...
            test_video_url = (
                "https://www.youtube.com/watch?v=jNQXAC9IVRw"  # 一个公开的测试视频
            )
            ...
            if result.returncode != 0:
                ...
                return {
                    ...
                    "region": None,
                    "error": error_msg,
                    "details": {"returncode": result.returncode, "stderr": error_msg},
                }
            ...
            try:
                ...
                return {
                    "available": False,
                    "region": None,
                    "error": f"{translate_log('cookie_test_parse_failed_error')}: {e}",
                    "details": {},
                }
            ...
            return {
                "available": False,
                "region": None,
                "error": translate_log("cookie_test_timeout"),
                "details": {},
            }
        except Exception as e:
            ...
            return {"available": False, "region": None, "error": str(e), "details": {}}
