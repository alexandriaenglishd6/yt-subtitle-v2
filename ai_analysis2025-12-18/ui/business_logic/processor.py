...
class VideoProcessor(
    VideoFetcherMixin, SubtitleDetectorMixin, TaskRunnerMixin, ProcessingPipelineMixin
):
    """视频处理器
    ...
    def __init__(self, config_manager: ConfigManager, app_config, event_bus=None, quiet: bool = False):
    ...
    def _init_components(self):
        ...
        if self.app_config.proxies:
            self.proxy_manager = ProxyManager(self.app_config.proxies, quiet=self.quiet)
            if not self.quiet:
                logger.debug(t("proxies_loaded", count=len(self.app_config.proxies)))
        else:
            self.proxy_manager = None
            if not self.quiet:
                logger.debug(t("no_proxy"))

        # 初始化 Cookie 管理器
        if self.app_config.cookie:
            self.cookie_manager = CookieManager(self.app_config.cookie)
            if not self.quiet:
                logger.debug(t("cookie_loaded"))
        else:
            self.cookie_manager = None

        # 初始化 VideoFetcher
        self.video_fetcher = VideoFetcher(
            proxy_manager=self.proxy_manager, cookie_manager=self.cookie_manager, quiet=self.quiet
        )
        ...
        translation_ai_config = profile_manager.get_ai_config_for_task(
            "subtitle_translate",
            fallback_config=self.app_config.translation_ai
        ...
        if translation_ai_config and translation_ai_config.enabled:
            try:
                ...
                profile_name = profile_manager.task_mapping.get(
                    "subtitle_translate", t("profile_default")
                )
                # 统一使用 DEBUG 记录初始化详情
                logger.debug(
                    t(
                        "translation_ai_client_init_success",
                ...
                from core.exceptions import map_llm_error_to_app_error

                self.translation_llm_init_error_type = map_llm_error_to_app_error(
                    e.error_type.value
                )
        else:
            self.translation_llm_client = None
        ...
        summary_ai_config = profile_manager.get_ai_config_for_task(
            "subtitle_summarize",
            fallback_config=self.app_config.summary_ai
        ...
        if summary_ai_config and summary_ai_config.enabled:
            try:
                ...
                profile_name = profile_manager.task_mapping.get(
                    "subtitle_summarize", t("profile_default")
                )
                # 统一使用 DEBUG 记录初始化详情
                logger.debug(
                    t(
                        "summary_ai_client_init_success",
