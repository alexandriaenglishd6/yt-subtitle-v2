...
│   ├── translator/      # 翻译器模块 (源字幕选择器)
│   ├── output/          # 格式化输出模块
│   ├── url_parser.py    # YouTube URL 解析器
│   └── pipeline/        # 业务流程编排
├── ui/                  # 现代化 GUI 实现
...
### v3.1.1 (2025-12-23)
- ✨ **代码拆分重构**：将大文件拆分为可复用模块，提升可维护性。
  - `translation_summary_page.py` → 提取 `AIConfigPanel` 组件
  - `translator.py` → 提取 `source_selector.py` 模块
  - `fetcher.py` → 提取 `url_parser.py` 模块
- 🔒 **API Key 安全性**：API Key 输入框始终显示脱敏格式，不再在焦点时显示完整 Key。
- 🐛 **语言文件清理**：清理 `zh_CN.json` 和 `en_US.json` 中的50+个重复键。
- 🐛 **Bug 修复**：修复测试连接时闭包变量捕获错误。
