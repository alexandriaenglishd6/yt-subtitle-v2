        ...
        for line in lines:
            ...
            if skip_header:
                ...
                if line_stripped.startswith("Kind:") or line_stripped.startswith(
                    ...
                    continue
                if (
                    line_stripped.startswith("Translator:")
                    or line_stripped.startswith("Reviewer:")
                    or line_stripped.startswith("المترجم:")
                    or line_stripped.startswith("المدقّق:")
                ):
                    continue
...
def merge_srt_entries(source_entries: List[Dict], target_entries: List[Dict]) -> str:
    """合并源语言和目标语言字幕条目
...
def merge_entries_to_txt(source_entries: List[Dict], target_entries: List[Dict]) -> str:
...
def write_txt_subtitle(srt_path: Path, txt_path: Path) -> Optional[Path]:
    ...
    try:
        ...
        logger.debug_i18n("txt_subtitle_written", file_name=txt_path.name)
        return txt_path
