...
from core.logger import get_logger, translate_log, translate_exception
...
class OutputWriter:
    ...
    def write_original_subtitle(
        self, video_dir: Path, subtitle_path: Path, source_language: str
    ) -> Path:
        """写入原始字幕文件
        ...
        try:
            ...
            if not _atomic_write(target_path, content, mode="w"):
                raise AppException(
                    message=translate_exception("exception.atomic_write_original_failed", path=str(target_path)),
                    error_type=ErrorType.FILE_IO,
                )
            logger.debug_i18n(
                "log.output_original_subtitle_written", file_name=target_path.name
            )
            return target_path
        except (OSError, IOError, PermissionError) as e:
            # 文件IO错误
            app_error = AppException(
                message=translate_exception("exception.write_original_subtitle_failed", error=str(e)),
                error_type=ErrorType.FILE_IO,
                cause=e,
            )
            logger.error(
                f"{translate_exception('exception.write_original_subtitle_failed', error=str(app_error))}",
    ...
    def write_translated_subtitle(
        self, video_dir: Path, subtitle_path: Path, target_language: str
    ) -> Path:
        """写入翻译字幕文件
        ...
        try:
            ...
            if not _atomic_write(target_path, content, mode="w"):
                raise AppException(
                    message=translate_exception("exception.atomic_write_translated_failed", path=str(target_path)),
                    error_type=ErrorType.FILE_IO,
                )
            logger.debug_i18n(
                "log.output_translated_subtitle_written", file_name=target_path.name
            )
            return target_path
        except (OSError, IOError, PermissionError) as e:
            # 文件IO错误
            app_error = AppException(
                message=translate_exception("exception.write_translated_subtitle_failed", error=str(e)),
                error_type=ErrorType.FILE_IO,
                cause=e,
            )
            logger.error(
                f"{translate_exception('exception.write_translated_subtitle_failed', error=str(app_error))}",
                extra={"error_type": app_error.error_type.value},
            )
            raise app_error
        except AppException:
    ...
    def write_summary(
        self, video_dir: Path, summary_path: Path, summary_language: str
    ) -> Path:
        """写入摘要文件
    ...
    def write_all(
        ...
        if original_path and original_path.exists() and source_lang:
            if language_config.subtitle_format == "txt":
                ...
                # 输出 SRT（srt 或 both）
                output_original_path = self.write_original_subtitle(
                    video_dir, original_path, source_lang
                )
        ...
        for target_lang, translated_path in translation_result.items():
            if translated_path and translated_path.exists():
                ...
                if language_config.subtitle_format == "txt":
                    ...
                    # 输出 SRT（srt 或 both）
                    output_translated_path = self.write_translated_subtitle(
                        video_dir, translated_path, target_lang
                    )
                    output_translated_paths[target_lang] = output_translated_path
                logger.info(
                    translate_log(
                        "translated_subtitle_written",
                        target_lang=target_lang,
                ...
                logger.warning_i18n(
                    "log.skip_writing_translated_subtitle",
                    target_lang=target_lang,
                    translated_path=str(translated_path) if translated_path else "None",
                    exists=translated_path.exists() if translated_path else False,
                    video_id=video_info.video_id,
                )
        ...
        if language_config.bilingual_mode == "source+target":
            ...
            if output_original_path and output_original_path.exists():
                ...
                if language_config.source_language:
                    ...
                    logger.debug(
                        translate_log(
                            "using_configured_source_lang", source_lang=source_lang
                        ),
                        video_id=video_info.video_id,
                    )
                ...
                if source_lang:
                    ...
                    for target_lang in language_config.subtitle_target_languages:
                        # 优先使用输出目录中的翻译字幕路径
                        output_translated_path = output_translated_paths.get(
                            target_lang
                        )

                        # 如果没有输出目录中的翻译字幕，尝试使用临时目录中的翻译字幕
                        if (
                            not output_translated_path
                            or not output_translated_path.exists()
                        ):
                            temp_translated_path = translation_result.get(target_lang)
                            if temp_translated_path and temp_translated_path.exists():
                                # 写入输出目录
                                output_translated_path = self.write_translated_subtitle(
                                    video_dir, temp_translated_path, target_lang
                                )
                                output_translated_paths[target_lang] = (
                        ...
                        if (
                            ...
                            official_path = download_result.get(
                                "official_translations", {}
                            ).get(target_lang)
                            if official_path and official_path.exists():
                                # 写入输出目录
                                output_translated_path = self.write_translated_subtitle(
                                    video_dir, official_path, target_lang
                                )
                                output_translated_paths[target_lang] = (
                                    output_translated_path
                                )
                                logger.info_i18n(
                                    "log.using_official_for_bilingual",
                                    target_lang=target_lang,
                                    video_id=video_info.video_id,
                                )
                        ...
                        if (
                            ...
                            or source_lang.split("-")[0] == target_lang.split("-")[0]
                        ):
                            if (
                                not output_translated_path
                                or not output_translated_path.exists()
                                ...
                                logger.info_i18n(
                                    "log.same_source_target_for_bilingual",
                        ...
                        if (
                            ...
                            logger.warning_i18n(
                                "log.no_target_subtitle_skip_bilingual",
                                target_lang=target_lang,
                                translation_result=str(translation_result.get(target_lang)),
                                official_translation=str(download_result.get('official_translations', {}).get(target_lang)),
                                video_id=video_info.video_id,
                            )
                            continue
                        ...
                        try:
                            ...
                            target_srt_for_bilingual = translated_srt_paths.get(
                                target_lang
                            )

                            # 如果没有临时目录中的翻译字幕，使用输出目录中的
                            if (
                                not target_srt_for_bilingual
                                or not target_srt_for_bilingual.exists()
                            ):
                                target_srt_for_bilingual = output_translated_path

                            logger.info(
                                translate_log(
                                    "bilingual_subtitle_start",
                                    source_lang=source_lang,
        ...
        if language_config.subtitle_format == "both":
            ...
            if (
                ...
                and output_original_path.exists()
                and output_original_path.suffix == ".srt"
            ):
            ...
            for target_lang, translated_path in output_translated_paths.items():
                if (
                    translated_path
                    and translated_path.exists()
                    and translated_path.suffix == ".srt"
                ):
            ...
            logger.debug_i18n(
                "log.output_txt_subtitle_generated", video_id=video_info.video_id
            )
        ...
        logger.info(
            translate_log("all_output_files_written", path=str(video_dir)),
            video_id=video_info.video_id,
        )
        return video_dir
    ...
    def write_bilingual_subtitle(
        ...
        target_path = (
            video_dir / f"bilingual.{source_language}-{target_language}{extension}"
        )
        ...
        try:
            ...
            if not source_subtitle_path.exists():
                raise AppException(
                    message=translate_exception("exception.source_subtitle_not_found", path=str(source_subtitle_path)),
                    error_type=ErrorType.FILE_IO,
                )
            if not target_subtitle_path.exists():
                raise AppException(
                    message=translate_exception("exception.target_subtitle_not_found", path=str(target_subtitle_path)),
                    error_type=ErrorType.FILE_IO,
                )
            ...
            if not source_content or not source_content.strip():
                raise AppException(
                    message=translate_exception("exception.source_subtitle_empty", path=str(source_subtitle_path)),
                    error_type=ErrorType.CONTENT,
                )
            source_entries = parse_srt(source_content)
            logger.debug(
                translate_log(
                    "parsing_source_subtitle",
            ...
            if not target_content or not target_content.strip():
                raise AppException(
                    message=translate_exception("exception.target_subtitle_empty", path=str(target_subtitle_path)),
                    error_type=ErrorType.CONTENT,
                )
            target_entries = parse_srt(target_content)
            logger.debug(
                translate_log(
                    "parsing_target_subtitle",
            ...
            if len(source_entries) == 0:
                ...
                raise AppException(
                    message=translate_exception(
                        "exception.source_subtitle_parse_empty_with_preview",
                        path=str(source_subtitle_path),
                        preview=repr(content_preview)
                    ),
                    error_type=ErrorType.CONTENT,
                )
            if len(target_entries) == 0:
                ...
                raise AppException(
                    message=translate_exception(
                        "exception.target_subtitle_parse_empty_with_preview",
                        path=str(target_subtitle_path),
                        preview=repr(content_preview)
                    ),
                    error_type=ErrorType.CONTENT,
                )
            ...
            if not _atomic_write(target_path, merged_content, mode="w"):
                raise AppException(
                    message=translate_exception("exception.atomic_write_bilingual_failed", path=str(target_path)),
                    error_type=ErrorType.FILE_IO,
                )

            logger.debug(
                translate_log("bilingual_subtitle_written", filename=target_path.name)
            )
            return target_path
            ...
            # 文件IO错误
            app_error = AppException(
                message=translate_exception("exception.write_bilingual_subtitle_failed", error=str(e)),
                error_type=ErrorType.FILE_IO,
                cause=e,
            )
            logger.error(
                f"{translate_exception('exception.write_bilingual_subtitle_failed', error=str(app_error))}",
                extra={"error_type": app_error.error_type.value},
            )
            raise app_error
        except AppException:
