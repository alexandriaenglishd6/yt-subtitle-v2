...
class StageQueue:
    ...
    def enqueue(self, data: StageData):
        ...
        try:
            ...
            logger.error_i18n(
                "log.stage_enqueue_failed", stage=self.stage_name, error=str(e)
            )
            raise
    ...
    def stop(self, timeout: float = 30.0):
        ...
        for worker in self.workers:
            ...
            if worker.is_alive():
                logger.warning_i18n(
                    "log.worker_thread_timeout", worker_name=worker.name
                )
        ...
        try:
            ...
            if self.stage_name == "detect":
                self.failure_logger.log_failure(
                    video_id=video_id,
                    url=url,
                    reason=reason or data.skip_reason or t("status_detecting") + t("status_failed"),
                ...
                self.failure_logger.log_download_failure(
                    video_id=video_id,
                    url=url,
                    reason=reason or t("exception.download_failed", reason=""),
                    error_type=error_type,
                    batch_id=data.run_id,
                    channel_id=data.video_info.channel_id,
                    channel_name=data.video_info.channel_name,
                ...
                self.failure_logger.log_translation_failure(
                    video_id=video_id,
                    url=url,
                    reason=reason or t("exception.translation_failed", reason=""),
                    error_type=error_type,
                    batch_id=data.run_id,
                    channel_id=data.video_info.channel_id,
                    channel_name=data.video_info.channel_name,
                ...
                self.failure_logger.log_summary_failure(
                    video_id=video_id,
                    url=url,
                    reason=reason or t("exception.summary_failed", reason=""),
                    error_type=error_type,
                    batch_id=data.run_id,
                    channel_id=data.video_info.channel_id,
                    channel_name=data.video_info.channel_name,
                ...
                self.failure_logger.log_failure(
                    video_id=video_id,
                    url=url,
                    reason=reason or t("exception.processing_failed", error=""),
                    error_type=error_type,
                    batch_id=data.run_id,
                    channel_id=data.video_info.channel_id,
                    channel_name=data.video_info.channel_name,
    ...
    def is_empty(self) -> bool:
        ...
        return (
            ...
            and self._processed_count + self._failed_count >= self._total_count
        )
