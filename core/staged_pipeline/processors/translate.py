    ...
    def process(self, data: StageData) -> StageData:
        ...
        try:
            ...
            if self.language_config.translation_strategy == "AI_ONLY" or self.force:
                ...
                # 优化：如果源语言和目标语言相同，则跳过翻译
                source_lang = data.download_result.get("original_lang")
                for target_lang in self.language_config.subtitle_target_languages:
                    if source_lang and _lang_matches(source_lang, target_lang):
                        logger.info_i18n(
                            "log.skip_translation_same_lang",
                            lang=target_lang,
                            video_id=vid,
                        )
                        # 如果有官方字幕，使用它；否则使用原始字幕
                        ...
                        if not official_path or not official_path.exists():
                            ...
                            for detected_lang, path in official_translations.items():
                                if _lang_matches(detected_lang, target_lang):
                                    official_path = path
                                    break
                        ...
                        if official_path and official_path.exists():
                            ...
                            # 使用原始字幕
                            translation_result[target_lang] = data.download_result.get(
                                "original"
                            )
                    else:
                ...
                if self.language_config.translation_strategy == "AI_ONLY":
                    logger.info_i18n(
                        "translation_strategy_ai_only",
                        languages=needs_translation,
                        video_id=vid,
                    )
                else:
                    logger.info_i18n(
                        "force_retranslate_mode",
                        languages=needs_translation,
                        video_id=vid,
                    )
            else:
                ...
                for target_lang in self.language_config.subtitle_target_languages:
                    ...
                    if not official_path or not official_path.exists():
                        for detected_lang, path in official_translations.items():
                            if (
                                _lang_matches(detected_lang, target_lang)
                                and path
                                and path.exists()
                            ):
                                official_path = path
                                logger.debug_i18n(
                                    "target_lang_fuzzy_match",
                    ...
                    if official_path and official_path.exists():
                        ...
                        translation_result[target_lang] = official_path
                        logger.info_i18n(
                            "target_lang_using_official",
                            target_lang=target_lang,
                            file_name=official_path.name,
                            video_id=vid,
                        )
                    elif (
                        data.download_result.get("original_lang")
                        and _lang_matches(data.download_result.get("original_lang"), target_lang)
                    ):
                        # 无官方字幕，但源语言和目标语言相同，跳过翻译，使用原始字幕
                        logger.info_i18n(
                            "log.skip_translation_same_lang",
                            lang=target_lang,
                            video_id=vid,
                        ...
                        needs_translation.append(target_lang)
                        logger.info_i18n(
                            "target_lang_no_official_need_ai",
                            target_lang=target_lang,
                            video_id=vid,
                        )
                ...
                if not needs_translation:
                    logger.info_i18n(
                        "all_target_langs_have_official",
                        strategy=self.language_config.translation_strategy,
                        video_id=vid,
                    )
            ...
            if needs_translation:
                logger.info_i18n(
                    "translation_step_start", languages=needs_translation, video_id=vid
                )
                if not self.translation_llm:
                    # LLM 不可用
                    needs_ai = self.language_config.translation_strategy in [
                        "AI_ONLY",
                    ...
                    if needs_ai:
                        ...
                        if self.translation_llm_init_error:
                            if self.translation_llm_init_error_type == ErrorType.AUTH:
                                warning_msg = translate_log(
                                    "log.translation_ai_init_failed_auth",
                                    error=self.translation_llm_init_error,
                                    languages=", ".join(needs_translation),
                                )
                            else:
                                warning_msg = translate_log(
                                    "log.translation_ai_init_failed_generic",
                                    error=self.translation_llm_init_error,
                                    languages=", ".join(needs_translation),
                                )
                        else:
                            warning_msg = translate_log(
                                "log.translation_ai_unavailable",
                                languages=", ".join(needs_translation),
                            )
                        ...
                        logger.warning_i18n(
                            "log.translation_strategy_official_only_no_subtitle",
                            languages=", ".join(needs_translation),
                            video_id=vid,
                        )
                else:
                    # LLM 可用，调用翻译
                    logger.info_i18n(
                        "calling_translator", languages=needs_translation, video_id=vid
                    )
                    translator = SubtitleTranslator(
                        llm=self.translation_llm, language_config=self.language_config
                    )
                    # 只翻译需要的语言
                    ...
                    logger.info(
                        translate_log(
                            "translator_result", languages=list(partial_result.keys())
                        ),
                        video_id=vid,
                    )
                    # 合并翻译结果（已有官方字幕的保持不变）
                ...
                logger.info_i18n(
                    "no_translation_needed_official",
                    languages=list(translation_result.keys()),
                    video_id=vid,
                )
            ...
            # 检查是否有翻译结果
            any(
                path and path.exists() for path in translation_result.values()
            )
            ...
            missing_languages = [
                target_lang
                for target_lang in self.language_config.subtitle_target_languages
                if not translation_result.get(target_lang)
                or not translation_result[target_lang].exists()
            ]
            ...
            if missing_languages:
                ...
                if self.language_config.translation_strategy == "OFFICIAL_ONLY":
                    ...
                    error_msg = translate_log(
                        "log.translation_strategy_official_only_missing",
                        languages=missing_str,
                    )
                    ...
                    data.error = AppException(
                        message=error_msg, error_type=ErrorType.CONTENT
                    )
                    return data
                else:
                    # 其他策略下，翻译失败不视为整体失败，继续处理
                    logger.warning_i18n(
                        "translation_missing_languages",
                        languages=missing_str,
                        video_id=vid,
                    )
                    if not self.dry_run:
                        ...
                        error_type = (
                            self.translation_llm_init_error_type
                            if self.translation_llm_init_error_type
                            else ErrorType.UNKNOWN
                        )
                        # 构建失败原因
                        if self.translation_llm_init_error:
                            if error_type == ErrorType.AUTH:
                                reason = translate_log(
                                    "log.translation_ai_init_failed_auth_missing",
                                    error=self.translation_llm_init_error,
                                    languages=missing_str,
                                )
                            else:
                                reason = translate_log(
                                    "log.translation_ai_init_failed_generic_missing",
                                    error=self.translation_llm_init_error,
                                    languages=missing_str,
                                )
                        else:
                            reason = translate_log(
                                "log.translation_failed_missing", languages=missing_str
                            )
            ...
            logger.info(
                translate_log("translation_complete_video", video_id=vid), video_id=vid
            )
            return data
            ...
            app_error = AppException(
                message=translate_exception("exception.translate_subtitle_failed_with_error", error=str(e)),
